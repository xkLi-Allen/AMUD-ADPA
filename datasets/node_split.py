    ...
    if split == "official":
        ...
        if name in have_split:
            if name == 'wikics':
                ...
                # val_masks = val_mask.t().contiguous()
                ...
                # stopping_masks = stopping_masks.t().contiguous()
            ...
            elif name in ('chameleondir', 'squirreldir'):
                train_masks, val_masks, test_masks = [], [], []
                for f in official_split:
                    tmp = np.load(f)
                    train_masks += [torch.from_numpy(tmp['train_mask']).to(torch.bool)]
                    val_masks += [torch.from_numpy(tmp['val_mask']).to(torch.bool)]
                    test_masks += [torch.from_numpy(tmp['test_mask']).to(torch.bool)]
                train_masks = torch.stack(train_masks, dim=0)
                val_masks = torch.stack(val_masks, dim=0)
